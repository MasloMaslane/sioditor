...
test.describe('c++ toolchain', () => {
  ...
  const downloadCpp = async (page: import('@playwright/test').Page) => {
    await page.goto('/');
  ...
  const setSource = async (page: import('@playwright/test').Page, source: string) => {
  ...
  test('compiles and runs a program reading stdin', async ({ page }) => {
    ...
    await expect(page.locator('.console pre')).toHaveText(/^10\s*$/, { timeout: 300_000 });
  ...
  test('reports compiler errors against the right line', async ({ page }) => {
    ...
    await expect(page.locator('.console pre')).toContainText('undeclared_thing', {
      timeout: 300_000,
    });
    await expect(page.locator('.status')).toContainText('blad kompilacji');
  });
  ...
  test('warns that `long` is 32-bit here and 64-bit on the judge', async ({ page }) => {
    ...
    await expect(page.locator('.console pre')).toContainText('ILP32', { timeout: 300_000 });
  });
  ...
  test('explains that exceptions are unavailable, instead of a raw link error', async ({
    ...
    await expect(page.locator('.console pre')).toContainText('Wyjatki', { timeout: 300_000 });
    await expect(page.locator('.console pre')).not.toContainText('\x1b[', { timeout: 5_000 });
  });
});
